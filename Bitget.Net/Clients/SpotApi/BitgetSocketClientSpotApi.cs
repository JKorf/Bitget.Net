...
namespace Bitget.Net.Clients.SpotApi
{
    ...
    public class BitgetSocketClientSpotApi : SocketApiClient, IBitgetSocketClientSpotApi
    {
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToTickerUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<BitgetTickerUpdate>> handler, CancellationToken ct = default)
        {
            ...
            return await SubscribeInternalAsync(BaseAddress.AppendPath("spot/v1/stream"), symbols.Select(s => new Dictionary<string, object>
                    {
                        { "instType", "SP" },
                        { "channel", "ticker" },
                        { "instId", s },
        ...
        {
            return await SubscribeInternalAsync(BaseAddress.AppendPath("spot/v1/stream"), new object[] { new Dictionary<string, object>
                    {
                        { "instType", "spbl" },
                        { "channel", "account" },
                        { "instId", "default" },
