...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `IBitgetSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new BitgetRestClient();
...
* Version 4.6.0 - 24 Sep 2026
    ...
    * Shared APIs
      ...
      * Added `(I)BitgetSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `BitgetOptions` to provide SharedApi related options
    ...
    * Added UnifiedApi.Account.GetFundingFinancialRecordsAsync endpoint
    * Added UnifiedApi.Account.GetSubTransferRecordsAsync endpoint
    * Added marginMode, longLeverage and shortLeverage to UnifiedApi.Acount.SetLeverage endpoint
    * Added Other value to ProductCategory enum
    * Updated leverage parameter to nullable in UnifiedApi.Account.SetLeverage endpoint
    * Updated InviterId and ParentId to nullable in BitgetUaAccountInfo model
    * Updated TotalFundingFee to nullable in BitgetUaPositionUpdate model
    * Updated fromAsset and toAsset parameters to nullable in UnifiedApi.Account.GetConvertRecordsAsync
    * Updated MaxOrderQuantity, FundInterval, MaintainTime and MaxMarketOrderQuantity to nullable in BitgetUaFuturesSymbol model
    * Fixed UnifiedApi.Trading.GetPositionsAsync returning null instead of empty list

* Version 4.5.0 - 21 Aug 2026
    * Updated to CryptoExchange.Net v12.5.0
    * Added UTA copy futures endpoints
    * Added MakerFeePercentage, TakerFeePercentage, LowerPriceLimitPercentage, UpperPriceLimitPercentage to SharedFuturesSymbol
    * Added LowerPriceLimitPercentage, UpperPriceLimitPercentage to SharedSpotSymbol
    * Added ClientOrderId property to BitgetFuturesUserTradeUpdate model
    * Added ClientOrderId to Shared SubscribeToUserTradeUpdatesAsync mapping
    * Fixed classic account get account list deserialization
